...
function SectionShell({ meta, mode, primaryKey, children }: { meta: SectionMeta; mode: ViewMode; primaryKey: PreferenceKey; children: React.ReactNode }) {
  ...
  return <article className={`product-section ${isPrimary ? 'is-primary' : ''} section-${meta.key}`} style={{ '--section-order': order, '--section-span': isPrimary ? 8 : mode === 'adaptive' ? 4 : 6 } as React.CSSProperties}><div className="section-heading"><div className="section-icon"><meta.icon size={17} /></div><div><span>{meta.eyebrow}</span><h3>{meta.title}</h3></div>{isPrimary && <span className="priority-pill"><SparkleMark /> 主要偏好</span>}</div>{children}</article>
}
...
export function ProductSections({ product, mode, primaryKey }: ProductSectionsProps) {
  return <div className={`product-sections ${mode === 'adaptive' ? 'is-adaptive' : ''}`}>
    <SectionShell meta={sectionMeta[0]} mode={mode} primaryKey={primaryKey}>
      <div className="price-layout"><div><span className="original-price">原價 <s>NT${product.originalPrice.toLocaleString()}</s></span><div className="current-price">NT${product.currentPrice.toLocaleString()}</div><div className="save-badge">現省 NT${(product.originalPrice - product.currentPrice).toLocaleString()}</div></div><div className="coupon-box"><Tag size={15} /><div><span>優惠券已解鎖</span><strong>- NT${product.coupon.toLocaleString()}</strong></div><ChevronRight size={16} /></div></div>
      <div className="effective-price"><span>使用優惠券後價格</span><strong>NT${(product.currentPrice - product.coupon).toLocaleString()}</strong><small>未含會員優惠</small></div>
      <div className="seller-comparison"><UsersRound size={15} /><span><strong>{product.sellerCount} 家賣家</strong>提供這項商品</span><button type="button">比較賣家 <ChevronRight size={14} /></button></div>
    </SectionShell>
    <SectionShell meta={sectionMeta[1]} mode={mode} primaryKey={primaryKey}>
      <div className="rating-hero"><strong>{product.rating}</strong><div><div className="stars">★★★★★</div><span>{product.reviews.toLocaleString()} 則已驗證評論</span></div></div>
      <div className="trust-metrics"><div><span>賣家評分</span><strong>{product.sellerRating}%</strong></div><div><span>買家照片</span><strong>{product.buyerPhotos}</strong></div><div><span>已驗證</span><strong><BadgeCheck size={16} /></strong></div></div>
      ...
      <button className="text-action" type="button">查看全部評論 <ChevronRight size={14} /></button>
    </SectionShell>
    <SectionShell meta={sectionMeta[2]} mode={mode} primaryKey={primaryKey}>
      <div className="spec-feature"><Wrench size={16} /><div><span>相容性</span><strong>{product.compatibility}</strong></div></div>
